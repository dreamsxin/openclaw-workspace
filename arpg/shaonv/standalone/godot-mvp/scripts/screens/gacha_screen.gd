...
func show_gacha() -> void:
	...
	app._view_container().add_child(app._panel(Vector2(0, 0), Vector2(1280, 720), Color(0.018, 0.014, 0.016, 0.14)))
...
func draw_top_buttons() -> void:
	app._view_container().add_child(app._panel(Vector2(42, 40), Vector2(404, 80), Color(0.026, 0.020, 0.018, 0.50)))
	...
	app._view_container().add_child(skip)

func draw_pool_tabs(realm: String) -> void:
	app._view_container().add_child(app._panel(Vector2(972, 126), Vector2(246, 470), Color(0.026, 0.020, 0.019, 0.62)))
...
func draw_pool_panel(pool: Dictionary, realm: String) -> void:
	...
	app._view_container().add_child(app._panel(Vector2(216, 198), Vector2(420, 196), Color(0.018, 0.014, 0.012, 0.50)))
...
func draw_wish_panel(pool: Dictionary, realm: String) -> void:
	app._view_container().add_child(app._panel(Vector2(214, 420), Vector2(330, 70), Color(0.035, 0.027, 0.023, 0.52)))
	...
	for i in range(2 if realm == "present" else 1):
		...
		app._view_container().add_child(app._panel(slot_pos, Vector2(70, 70), Color(0.09, 0.072, 0.058, 0.82)))
		var add = app._label("+", 30, HORIZONTAL_ALIGNMENT_CENTER)
		add.position = slot_pos + Vector2(0, 14)
		add.size = Vector2(70, 34)
		app._view_container().add_child(add)
