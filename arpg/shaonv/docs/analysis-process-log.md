...
- Godot Gal 主界面按 `现世界面.jpg` 复核后确认：房间背景不是 `gal_img_122`，而是 `gal_bg_room_4`；`hero_037r` / `hero_037r_s01` 是 Gal 专用 spine 线索但当前 baked 渲染会散片化，MVP 暂用可正常拼装的 `hero_037`，经验记录见 `shaonv-gal-main-panel-wireframe-2026-05-25.md`。
