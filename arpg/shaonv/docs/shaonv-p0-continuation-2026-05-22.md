...
- 抽卡：普通/高级/进阶/源神祈愿池切换，单抽和十连；抽卡按钮先进入喚靈演出/确认页，再进入结果页。
- 抽卡规则：`gacha_pools_mvp.json` 驱动概率、UP 权重、保底、重复碎片和 Static 来源标注。
- 结果：主出货角色、稀有度标题、模拟稀有度光效层、结果卡片色框、新角色标记、重复转化提示，点击结果可进入角色详情或设为看板。
- 图鉴：已获得角色高亮，未获得灰显，显示持有数量、碎片和收集进度；支持全部/已获得/未获得/稀有度筛选，并按已获得优先、稀有度优先排序。
- 角色详情：展示获得状态、稀有度、碎片、获得途径、资源路径和 Spine key；未获得角色叠加锁定遮罩，已获得角色可设为看板。
- 记录：最近抽卡记录。
