...
`UIInGame` now has a first gameplay shell in `run-game.bat` mode. `InGameReferenceShell` reads the recovered `UIInGame` RectTransforms and draws the top wallet bar plus candidate `Request`, `Bottom/UIBlockInfo`, inventory, and lobby regions behind the playable board. In the 540 x 960 portrait window, the 7x7 board is centered and the older desktop debug/reference panels are hidden so the restored path remains playable after `UIOutGame/InGameBtn`. The shell uses committed background and currency sprites where available. The legacy standalone Produce button is hidden in portrait mode. The top request strip now reconstructs first-pass `Request/RequestList` structure with a quest card, reward slots, and `SkillInfo` placeholders. The bottom operation bar reconstructs the original `Bottom` proportions: 150-wide Bag/Cafe side buttons, a central 160-high `UIBlockInfo` strip, and first-pass `Btn_BoxOpen`, `Btn_Use`, and `Btn_CoolTime` operation buttons. Produce calls the current recovered producer logic through the `Btn_BoxOpen` region, Bag opens a first-pass top-level `UIPopup_Inventory` structural shell with `ProduceInventory` and `NormalInventory` slot sections, and Cafe returns to `UIOutGame`. The popup currently visualizes selected/board block summaries; persistent `InvenDataManager` slot serialization is still pending.
