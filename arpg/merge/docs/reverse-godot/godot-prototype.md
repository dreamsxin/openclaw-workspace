...
The loading-screen reference layer is shown by default on startup, and the left-side `Loading Ref` button toggles it. `run-game.bat` starts the project in restored startup mode, passes `--restored-startup`, and displays this loading layer full-window in a portrait-oriented 540 x 960 window. It uses the recovered `UILoading` RectTransforms for the 1080 x 1920 root, background layers, logo area, loading bar, and version-label corners. The current layer now uses recovered PNGs for the background/logo/loading fill where available, while Spine logo renderer behavior is still represented as a static image fallback.
