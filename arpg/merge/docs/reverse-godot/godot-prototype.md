...
`UILoading` uses the recovered RectTransforms for the 1080 x 1920 root, background layers, logo area, loading bar, and version-label corners. `UISceneLoading` uses the recovered root/background structure and now draws `SkeletonGraphic (kokomi_Loading)` through a first-pass Spine atlas-region animation: the original `.atlas.txt`, `.skel.bytes`, and texture pages are copied under `godot-project/assets/spine/loading/kokomi_Loading/`, then Godot crops named regions and animates body, hair, eyes, arms, skirt, and table layers. This is not yet full Spine binary timeline playback; raw atlas pages are only used as crop sources and are not drawn as complete character art.
