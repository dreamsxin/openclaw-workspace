...
`UILoading` uses the recovered RectTransforms for the 1080 x 1920 root, background layers, logo area, loading bar, and version-label corners. `UISceneLoading` uses the recovered root/background structure and now draws `SkeletonGraphic (kokomi_Loading)` through a generated Godot Spine bridge rig. The original `.atlas.txt`, `.skel.bytes`, and texture pages are copied under `godot-project/assets/spine/loading/kokomi_Loading/`; `kokomi_Loading.rig.json` then defines bones, attachments, draw order, and loading-idle channels so Godot can evaluate a small skeletal pose graph before cropping named atlas regions. This is not yet exact Spine binary timeline playback; raw atlas pages are only used as crop sources and are not drawn as complete character art.
