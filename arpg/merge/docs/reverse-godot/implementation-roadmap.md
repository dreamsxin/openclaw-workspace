...
| T027 | Implement Spine character render path | in progress | `kokomi_Loading` now uses a generated Godot rig JSON with bones, attachments, draw order, and idle channels; full `.skel.bytes` binary skeleton/timeline playback still pending |
