...
| T027 | Implement Spine character render path | in progress | `kokomi_Loading.skel.bytes` is now parsed by official `@esotericsoftware/spine-core@4.2.43`; Idle is baked to exact world-vertex frames and rendered in Godot; reusable runtime path and Interaction playback still pending |
