...
| T052 | Add UIPopup_MaidLobbySelect shell | pending | Rebuild the recovered `UIPopup_MaidLobbySelect` panel with `TextTitle`, `Btn_Close`, `MaidList`, and maid list-item states; wire Select from `UIMaidLobby` |
| T053 | Add maid interaction/dialog shell | pending | Use `UIMaidLobby.OnClick_ShowDialog`, `UIOutGame/Btn_ToInteraction`, `Npc_Dialog`, and decoded dialog rows to build the first talk/interaction popup; replace placeholder text with real localized dialog candidates |
