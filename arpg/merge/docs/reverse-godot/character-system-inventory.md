...
| `SpineCharacterRenderer` | First pass exists for `UISceneLoading/kokomi_Loading`; official Spine runtime baking now emits `Idle` and `Interaction` clips consumed by Godot textured-polyon playback; reusable preview renderer now drives the all-character browser, while direct UIOutGame/UIMaidLobby embedding is pending |
...
5. Reuse `SpineBakedPreviewCanvas` in the actual UI screens that need live character display: `UIOutGame/UIMaidLD`, `UIMaidLobby/SpinePos`, and `UIPopup_MaidLobbySelect`.
