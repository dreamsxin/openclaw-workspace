...
`UISceneLoading` now has a Godot reference layer in restored startup mode. It maps the recovered full-stretch root and background candidates and renders the original `SkeletonGraphic (kokomi_Loading)` target with a generated Spine bridge rig. The current implementation imports `kokomi_Loading.atlas.txt`, `kokomi_Loading.skel.bytes`, `kokomi_Loading.png`, and `kokomi_Loading_2.png` from the AssetRipper export, generates `kokomi_Loading.rig.json`, evaluates bones/attachments/loading-idle channels in Godot, and crops named atlas regions for the preview. Full `.skel.bytes` timeline playback is still a follow-up; raw atlas page PNGs remain evidence/crop sources only and should not be drawn directly as final character art.
