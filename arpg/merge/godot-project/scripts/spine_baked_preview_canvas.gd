...
func set_baked_path(baked_path: String, preferred_clip := "") -> void:
	...
	for page_name in pages.keys():
		var texture_path := String(pages[page_name])
...
func _draw() -> void:
	...
	draw_rect(target, Color(0.045, 0.048, 0.055, 1.0), true)
	_draw_reference_grid(target)
	if not load_error.is_empty():
		_draw_center_text(load_error, Color(1.0, 0.62, 0.48, 0.95))
		return
	if baked.is_empty():
		_draw_center_text("No character selected.", Color(0.82, 0.86, 0.9, 0.86))
		return
	var clip := _active_clip()
	if clip.is_empty():
		_draw_center_text("No baked clip.", Color(1.0, 0.72, 0.56, 0.92))
		return
	_draw_baked_clip(clip, target.grow(-24.0))

func _draw_reference_grid(target: Rect2) -> void:
	var color := Color(1, 1, 1, 0.045)
	var step := 48.0
	var x := fmod(-time * 10.0, step)
	while x < target.size.x:
		draw_line(Vector2(x, 0), Vector2(x, target.size.y), color, 1.0)
		x += step
	var y := 0.0
	while y < target.size.y:
		draw_line(Vector2(0, y), Vector2(target.size.x, y), color, 1.0)
		y += step
...
func _draw_baked_clip(clip: Dictionary, target: Rect2) -> void:
	...
	var fps := float(clip.get("fps", baked.get("bake", {}).get("fps", 10.0)))
	...
	if bounds.size.x <= 0.0 or bounds.size.y <= 0.0:
		return
	var scale_factor := minf(target.size.x / bounds.size.x, target.size.y / bounds.size.y) * 0.92
