...
- 2026-05-20 英雄详情继续按源码核对：英雄背包点击进入 `HeroMainPre`，图鉴点击进入 `HeroBookDetailPre`，两者共享很多节点名但坐标略有差异，不能固定一套坐标。`btnPre/btnNext/ft_zhanli/lblHeroName/lblNickName/lblNickname` 已改为按当前 `detail_mode` 从对应 prefab layout 读取；主培养页角色目标框优先使用 `HeroMainPre.skinBodyBox`。后续继续把 `scrollview/skinInfo/skinBox` 等节点也迁到同一套 mode-aware layout 查询。
