...
| 主城聊天 | `MainUIPanel.onShow()` 绑定 `this.com.chat.getChildByName("btn") -> openChatPanel()`；聊天面板源码 `ChatPanel.preUrl="Prefab/Chat/ChatPanelPre"` | `ChatPanelPre` + `ChatMainItemPre` | 主屏聊天栏 -> prefab 预览 | 已导出 `data/prefab_layouts/ChatPanelPre.json` / `ChatMainItemPre.json`，并给 Godot 主屏聊天栏补透明热区。 |
| 主城立绘点击 | `MainUIPanel.onShow()` 绑定 `this.com.lihui -> openLhPanel()`，打开主城立绘/背景切换层；同时 `showLh()` 用 `RoleLh` 加载 `Prefab/HerolhPrefab/<bodyID>` | `MainPre.lihui` + `lihuiPanel` + `Prefab/HerolhPrefab/105004` | 当前保留为 Spine 动作切换调试热区 | 已确认源码入口，但暂不覆盖当前点击动作切换；后续应单独增加“立绘/背景”入口或长按/调试按钮，避免破坏 Spine 动作检查。 |
...
| 聊天 | `Prefab/Chat/ChatPanelPre` | 主城底部聊天栏 `chat.btn -> openChatPanel()`，已导出 `ChatPanelPre.json` 和主城消息条目 `ChatMainItemPre.json`；Godot 主屏聊天栏现在可点击进入 prefab 预览。 |
...
- 2026-05-20 主城入口补查：`MainUIPanel.onShow()` 里聊天不是静态文本，真实绑定是 `chat.btn -> openChatPanel()`；`ChatPanel.preUrl="Prefab/Chat/ChatPanelPre"`，聊天条目由 `showMainChat()` 动态实例化 `Prefab/Chat/ChatMainItemPre`。`tools/export_cocos_prefab_layout.py` 已加入这两个 prefab 并重新导出；Godot 主屏聊天栏补透明热区，点击进入“聊天” prefab 预览。立绘 `lihui -> openLhPanel()` 也已确认，当前暂保留为 Spine 动作切换热区，后续再独立接入立绘/背景切换层。
- 2026-05-20 主城左侧活动区补查：`MainPre.hudong` 父节点带 `cc.Layout`，源码 `onfrist()` 先把 `libao/fuli/huodong/kaifu/ghz/skin/wangzhe/dagong/tianti/cymz/pass` 等设为 inactive，再由活动状态、红点包和倒计时逐个打开，Cocos 会重新布局 active 子节点。Godot 不能把 inactive 节点的导出坐标直接照搬，否则会出现多个活动图标占同一格；当前手工主屏改为按 `huodong` 父区域 5 列网格重排，并用 `cc.SpriteFrame` 索引按逻辑路径加载真实图标。
- 2026-05-21 主城左侧快捷按钮补查：`MainPre.json` 中 `zjm_btn_HaoYou/zjm_btn_YouJian/zjm_btn_PaiHang/zjm_btn_XinWen/zjm_btn_ZhanBao` 的导出 SpriteFrame 存在串图，可靠资源在 `data/config_index/by_type/cc.SpriteFrame.json` 的 `image/com/mainpanel/zjm_btn_*` 路径。Godot 已改为按逻辑路径加载好友、邮件、排行、客服、战报、绑定平台图标；源码 `onShow()` 明确 `discordBtn.active=false`，本地默认也隐藏 Discord 活动入口。
- 2026-05-21 主城右侧系统入口补查：`MainPre` 右侧入口按钮和图标导出坐标可用，但后几项图标节点在 prefab 中复用 `zjm_icon_zhaohuan`，不能直接当作最终图标。可靠资源仍在 `cc.SpriteFrame` 索引：`zjm_icon_yinghun/duanzao/zhanbu/xunxing/shanghui` 分别对应英魂、锻造、占卜、寻星、商会。Godot 已改为右侧入口背景和图标都优先按逻辑路径加载，减少手工 atlas 矩形兜底。
- 2026-05-21 底部导航资源链复查：`assets/resources/import/0f/0fc983b1-cd2b-4115-aa37-5b71c0e1ad5f.json` 是 `daohangPre` 原始压缩 prefab，`DaohangPanel.changeTabPanel()` 明确 `btn1..btn6` 分别路由城镇、英雄、召唤、冒险、冒险地图/副本、公会。`daohangPre.json` 导出的 `cm_tab_ZhaoHuan` 节点尺寸和坐标是正确的 `[481.599,591.329,125,123]`，但 SpriteFrame 被解析成了右侧小图标 `image/com/mainpanel/zjm_icon_zhaohuan` `[707,551,34,34]`，这是导出器串图，不应直接使用。Godot 主屏底栏已改为：城镇/英雄/冒险/副本/公会按 `cc.SpriteFrame` 索引中的 `image/com/mainpanel/cm_icon_*` 加载；召唤暂按原始 `NAV_SLOT3_RECT=[477,242,125,123]` atlas 区域兜底，后续若接 `spimg/sp1..sp6` 动效，再从原始 prefab 里的 `uispine/main/*` 和召唤按钮 Spine 继续恢复。
- 2026-05-21 底部导航一致性修正：英雄列表页 `original_hero_list_panel.gd` 和英雄详情页 `original_hero_panel.gd` 原本各自手写底栏，其中英雄列表仍用 `image/com/mainpanel/zjm_icon_zhaohuan` 的 34x34 小图，英雄详情用另一个 80x80 atlas 区域；两者都和 `DaohangPanel` 的底栏召唤位不一致。现已统一为主屏复核出的 `NAV_SUMMON_RECT=[477,242,125,123]`，路由仍进入 `original_draw_card_panel.tscn`。旧 `original_main_city.gd` 的 `SUMMON_RECT=[707,551,34,34]` 属于右侧入口小图标，不是底栏图标，暂不替换。
- 2026-05-21 底部导航坐标继续收敛：英雄列表页和英雄详情页不应使用等距手写中心点。源码 `DaohangPanel` 由 `daohangPre` 按 1280x720 全屏坐标渲染，热区 `btn1..btn6` 均是 120x120，图标节点分别为城镇 `[115.645,585.329,152,141]`、英雄 `[284.102,583.476,150,142]`、召唤 `[481.599,591.329,125,123]`、冒险 `[645.211,582.972,150,145]`、副本 `[842.368,589.329,134,133]`、公会 `[1027.648,592.829,119,126]`。Godot 英雄列表/详情页已改为使用这些 screen_rect，并补真实 `cm_menu_BeiJing` 底栏背景；按钮只保留透明热区，不再绘制 Godot 默认按钮底色。
- 2026-05-21 抽卡页坐标复查：`drawCardPre.json` 中 `btn_call1=[55,423,238,66]`、`btn_call10=[55,519,238,66]`、`btn_dh=[56.384,351,300,34]`，当前 Godot 旧实现把召唤按钮放在右下小面板内，位置和尺寸都偏离 prefab。已先把召唤积分条与“召唤1次/10次”按钮改回导出 screen_rect；按钮背景纹理仍需后续继续追 `btn_call1/btn_call10/Background` 的九宫格 SpriteFrame。
- 2026-05-21 英雄详情衣装逻辑复查：源码 `HeroMainPanel.showTab()` 切到 tab5 时激活 `skinBox` 并请求 `CG_SKIN_HERO()`；`setSkinData()` 根据 `skinData.ind/isOn` 切换 `getBtn/wearBtn/takeBtn`，`playBtn` 进入 `SkinShowPre`，`btnNext` 切下一套衣装。Godot 侧已补本地衣装闭环：内置演示英雄补可用 `skins`，衣装 tab 选择皮肤时主舞台同步显示当前 skin body，穿戴/卸下会本地保存并影响非衣装 tab 的主预览，展示按钮跳到 `original_draw_hero_show.tscn` 并携带 `skin_body` 优先显示 `image/skin/showImg/<body>`。
- 2026-05-21 英雄详情培养页继续复查：`HeroMainPre.json` 的培养页属性节点可直接使用，关键坐标为 `lblGongJiAttr=[970.389,169.239,73.41,27.72]`、`lblShengMingAttr=[970.389,209.803,73.41,27.72]`、`lblFangYuAttr=[970.389,249.369,73.41,27.72]`、`lblSuduAttr=[970.389,290.761,73.41,27.72]`、`mask1=[944.414,393.77,170,32]`、`mask2=[942.753,450.697,173,21]`、`lblLvValue=[984.159,438.857,90.75,34.24]`。Godot 已把培养 tab 从通用摘要改为按这些 screen_rect 渲染，并降低详情面板遮罩透明度，避免覆盖 `yx_img_PeiYang/yx_skin_bg` 等真实 tab 背景。左侧按钮按截图/源码运行态保留“查看/分享/锁定”，其中查看对应源码 `btnChaKan()` 的隐藏 UI 全屏预览逻辑。
- 2026-05-21 英雄详情子 prefab 接入：源码 `HeroMainPanel.showTab()` 的 tab2/tab3/tab4 分别懒加载 `Prefab/HeroPanel/zhuangbeiBox`、`shengxingBox`、`zhanyiBox`。Godot 已读取 `data/prefab_layouts/zhuangbeiBox.json`、`shengxingBox.json`、`zhanyiBox.json`，装备 tab 的 `equipBox1..6/fuwenIcon1..2/btnChuanDai`、升星 tab 的 `btnUpStar/btnYHD/lblVal*/lblHeroWord*`、战意 tab 的 `zhanyi0..2/btnYuLan/btnHelp` 开始走导出的 screen_rect。注意 `shengxingBox` 中部分 `heroBox` 容器导出到 1100/1300 区域，是父节点偏移导致，材料图标仍需按实际显示区域手工兜底，文字和按钮坐标可直接使用。
- 2026-05-21 英雄详情点击逻辑继续对齐：`equipBoxClick()` 中 1-4 号装备设置 `curEquipType` 后打开装备 tip 或 `HeroEquipChangePre`，5 号水晶未持有时调用 `CG_SHUIJING_JIHUO`、已持有时 `openShuiJingGridTip`，6 号进入神器链路；`onFuWenGridClick()` 按 `lockFuwen` 决定显示“达到100级解锁/英雄达到七星时解锁”或打开符文选择/tip。Godot 装备 tab 已拆成 `_on_equipment_slot_pressed()` 和 `_on_fuwen_slot_pressed()`，保留离线提示并路由到对应 prefab 预览。升星材料点击源码 `clickHero()` 会打开 `HeroPaleceSelectMaterialsPre`，Godot 已给前三个英雄材料补热区，第 4 个道具材料保留获取入口提示。战意 tab 修正了锁定文字索引，避免把局部坐标误显示成文案。
