...
- 2026-05-20 英雄列表第三轮：按 `HeroListPre` 导出的 `btnTypeAll/btnType1...btnType5` 精确坐标重排顶部阵营筛选，右侧 `btnHero/btnBook/btnShared/btnYingHun/btnNormalarray/btnStar` 改为原始 `198x72` 热区；英雄卡从 86 调到 92 头像区，保留 7 列密度以贴近原截图。源码 `btnYingHun -> openYinghun() -> PanelManager.openHeroPalacePanel()`，已从本页碎片 mock 改为跳转 `英魂殿/HeroPalacePre` prefab 预览。`image/common/cm_tab1_on/off` 在 config 中存在但未进入 `named_resource_index`，当前右侧页签用手工 StyleBox 代替，后续需要补通 common SpriteFrame 索引。`HeroPalacePre` 预览发现 `assets/resources/native/1d/1d9c822a-b0c7-4347-a697-1017eac7c334.png` Godot 报 `ERR_FILE_CORRUPT`，属于资源转换/解包待修项。
