...
| `scenes/original_draw_card_panel.tscn` | `scripts/original_draw_card_panel.gd` | `Prefab/DrawCard/drawCardPre` | `DrawMainPanel` | 抽卡页有页签和部分 `ZhaoHuan_*` Spine；已按 `onDrawCardOpResult()` 补 `HeroUiBox` 十连结果 overlay，结果头像按 `bookPosArr` 近似排布，点击进入 `HeroShowPre` 预览。 | 继续把 `HeroShowPre` 从 prefab 预览收敛为手工单英雄展示，并补完整抽卡翻牌特效。 |
