...
| `scenes/original_server_select.tscn` | `scripts/original_server_select.gd` | `Prefab/login/pfLoginPanelPre` | `PFLoginPanel` | `PFLoginPanel` 是正式登录/选服入口；已按 `pfLoginPanelPre.json` 和 `PFLoginCom` 补回服务器浮框、`txtServer`、开始按钮、右侧公告/切换账号/Discord/Facebook、版号/适龄提示、隐私勾选与协议文案。背景改用 `uispine/denglu/bg`。服务器框/切换箭头/火爆/新服/维护标签来自 `image/com/login` SpriteFrame。已新增本地服务器列表弹层，参考 `nodeSv/svBg/scrollTab/scrollserver` 坐标；公告按钮已按 `nodeGG/gonggao_bg/scrollview/tmptxt` 坐标显示本地 mock 公告层；切换账号已按 `nodeAlert/cm_frame_TanChuang2/richtext/取消/确定` 补确认框；隐私协议点击区已按 `useprivacyPanel` 行为补本地面板；适龄提示已按 `shilingPanel` 行为补本地面板。点击开始游戏会按源码显示 `LoadingPre` 的 `isFist=1` 连接 alert 模式，再进入主城。 | 后续补 `useprivacyPre/shilingPre` 精确 layout 导出、选中勾选 SpriteFrame 和服务器列表 ScrollView 真实滚动/分组数据。 |
...
- 隐私协议源码为 `useprivacyPanel.preUrl="Prefab/loading/useprivacyPre"`，正文来自 `configs/useprivacy`，拒绝调用 `cheks.uncheck()`，同意调用 `cheks.check()`。当前 `useprivacyPre.json` 尚未导出，Godot 先用本地面板保留同意/拒绝行为；真实正文和精确按钮资源后续从 `configs/useprivacy` 与 `Prefab/loading/useprivacyPre` 补。
...
- 适龄提示源码为 `shilingPanel.preUrl="Prefab/loading/shilingPre"`，`onShow()` 将 `configManage.shilingTxt` 写入 `t1`，点击 `btn` 关闭。当前 `shilingPre.json` 尚未导出，Godot 先用 `cm_frame_TanChuang2` 本地面板保留展示/关闭行为。
