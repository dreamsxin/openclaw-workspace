...
- `HeroMainPanel.showTab()` 切 tab 时会替换 `thisPanel.imgBg`：培养 `yx_img_PeiYang`，装备 `yx_img_ZhuangBei`，升星 `yx_img_ShengXing`，战意 `yx_img_ZhanYi`，衣装 `yx_skin_bg`。源码会先隐藏旧内容再加载对应子 prefab，非培养页不能叠加培养摘要。当前 Godot `main` 模式已导出并接入 `Prefab/HeroPanel/zhuangbeiBox`、`shengxingBox` 与 `zhanyiBox`：装备页按 6 个装备位、2 个符文/神器位和“一键穿戴”坐标绘制；升星页按属性提升、材料位、英魂按钮和升星按钮坐标绘制；战意页按 3 个战意节点、“战意预览”和帮助按钮坐标绘制。衣装页使用 `HeroMainPre.skinBox` 的原始坐标，资源路径遵循源码 `image/skin/showImg/<body>`。
