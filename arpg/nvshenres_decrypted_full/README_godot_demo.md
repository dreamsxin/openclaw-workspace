...
- `HeroMainPanel.showTab()` 切 tab 时会替换 `thisPanel.imgBg`：培养 `yx_img_PeiYang`，装备 `yx_img_ZhuangBei`，战意 `yx_img_ZhanYi`，衣装 `yx_skin_bg`。当前 Godot `main` 模式已按 `selected_tab` 切换这些底图；装备和战意的真实子面板是 `Prefab/HeroPanel/zhuangbeiBox` 与 `Prefab/HeroPanel/zhanyiBox`，仍需先导出 layout 再继续替换手工内容。
